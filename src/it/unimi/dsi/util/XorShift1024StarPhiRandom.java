 ...
 * @see Random
 * @see XorShift1024StarPhiRandomGenerator
 */
public class XorShift1024StarPhiRandom extends Random {
	 ...
	 */
	public XorShift1024StarPhiRandom(final long seed) {
		super(seed);
	}
	...
	public void jump() {
		...
		for (final long element : JUMP)
			for (int b = 0; b < 64; b++) {
				if ((element & 1L << b) != 0) for (int j = 0; j < 16; j++)
					t[j] ^= s[(j + p) & 15];
				nextLong();
			}
	...
	public XorShift1024StarPhiRandom split() {
		nextLong();
		final XorShift1024StarPhiRandom split = new XorShift1024StarPhiRandom(0);
